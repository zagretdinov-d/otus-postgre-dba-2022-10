...
copy_data (boolean) определяет должны ли копироваться существующие данные в публикациях на которые оформляется подписка сразу после начала репликации. Значение по умолчанию — true. а я поставил false. 
Пересоздам подписки с условием true получения всех данных:
...
> теперь проблема сдесь заключается в том что невозможно настроить обычную master-slave репликацию. Нужно менять параметры на pg-node3. в результате репликация ломается. Перестаёт работать pg-node3. И в этом случае настрою 4 ую ноду.
